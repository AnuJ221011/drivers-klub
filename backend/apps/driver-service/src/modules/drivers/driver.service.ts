...
export class DriverService {
    ...
    async createDriver(data: CreateDriverInput, userScope?: any) {
        ...
        const user = await prisma.user.findFirst({
            where: {
                OR: [{ id: data.userId }, { shortId: data.userId }]
            },
        });
        ...
        const existing = await this.driverRepo.findByUserId(user.id);
        if (existing) {
        ...
        const payload =
            Object.keys(meta).length > 0
                ? { ...rest, providerMetadata: meta, userId: user.id }
                : { ...rest, userId: user.id };

        const shortId = await IdUtils.generateShortId(prisma, EntityType.DRIVER);

        return this.driverRepo.create({ ...payload, shortId });
    }
    ...
    async createNewDriver(data: CreateNewDriverInput) {
        ...
        // Create driver
        let driver = await this.driverRepo.createNewDriver(defaultFleetId, shortId, { ...data, userId: user.id });
        ...
        await this.referralService.ensureUserHasReferralCode(user.id);
    ...
    async resolveDriverId(id: string): Promise<string> {
        const driver = await this.driverRepo.findById(id);
    ...
    async updateDriver(id: string, data: UpdateDriverInput, userScope?: any) {
        ...
        return this.driverRepo.updateDetails(driver.id, update);
    }
    ...
    async updateDriverStatus(id: string, data: UpdateDriverStatusInput, userScope?: any) {
        ...
        return this.driverRepo.updateStatus(driver.id, data);
    }
    ...
    async updateDriverAvailability(
        ...
        return this.driverRepo.updateAvailability(driver.id, data);
    }

    async getDriverPreferences(id: string, userScope?: any) {
        const driver = await this.getDriverById(id, userScope);

        const driverPreference = await this.driverRepo.findDriverPreferences(driver.id);
    ...
    async driverPreferencesChangeRequest(id: string, data: any) {
        ...
        const driverPreferenceUpdateRequest =
            await prisma.driverPreferenceRequest.findFirst({
                where: { driverId: driver.id, status: PreferencesRequestStatus.PENDING },
            });
