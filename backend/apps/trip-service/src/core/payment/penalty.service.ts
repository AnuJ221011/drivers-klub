    ...
    private async handleRentalModelPenalty(penaltyId: string, driverId: string, amount: number, driver: any) {
        ...
        if (availableDeposit >= amount) {
            ...
            try {
                ...
                logger.error(`[PenaltyService] Failed to generate penalty payment link for driver ${driverId}:`, error);
                return {
    ...
    private async applyBlacklist(driverId: string, driver: any) {
        ...
        if (driver.depositBalance > 0) {
            try {
                ...
                logger.info(`[PenaltyService] Refund initiated for Driver ${driverId}: ₹${payoutTransaction.amount}`);
                // In production: Call EasebuzzAdapter.createPayout() here if account details exist.

            } catch (error) {
                logger.error(`[PenaltyService] Failed to initiate blacklist refund for driver ${driverId}:`, error);
            }
        }

        logger.info(`[PenaltyService] Driver ${driverId} blacklisted`);
    }
