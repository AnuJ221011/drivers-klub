...
export class TripService {
  ...
  async arriveTrip(tripId: string, driverUserId: string, lat: number, lng: number) {
    ...
    // Authorization: only the assigned driver can mark arrive
    const driver = await prisma.driver.findUnique({
      where: { userId: driverUserId }
    });
    const isAssigned = trip.tripAssignments.some((ta: any) => ta.driverId === driver?.id);
    if (!driver || !isAssigned) {
      throw new ApiError(403, "Not authorized to mark arrival for this trip");
    }
  ...
  async noShowTrip(tripId: string, driverUserId: string) {
    ...
    // Authorization: only the assigned driver can mark no-show
    const driver = await prisma.driver.findUnique({
      where: { userId: driverUserId }
    });
    const isAssigned = trip.tripAssignments.some((ta: any) => ta.driverId === driver?.id);
    if (!driver || !isAssigned) {
      throw new ApiError(403, "Not authorized to mark no-show for this trip");
    }
